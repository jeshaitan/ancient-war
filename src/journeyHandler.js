...
exports.main = function(req, res, sess, db) {
  ...
  //user has chosen biome, describe enemy
    ...
    if(req.body.input == 'woods' || req.body.input == 'Woods' || req.body.input == 'w' || req.body.input == 'W') {
      ...
      sess.history.push({type: 'journey', author: 'Server', description: 'You are attacked by level ' + sess.enemy.level + " " + sess.enemy.name + '!'},
                        {type: 'journey', author: 'Server', description: 'It has ' + sess.enemy.hp + ' health!'});
      sess.step++;
    }
      ...
      sess.step = null;
      sess.history.push({type: 'lobby', author: 'Server', description: 'Sorry, I don\'t understand that.'});
    }
  }
  //battle initiated, enter battle loop
    ...
    if(sess.enemy.hp == 0) {
      ...
      sess.history.push({type: 'journey', author: 'Server', description: 'You killed the ' + sess.enemy.name + '!'},
                        {type: 'journey', author: 'Server', description: 'You gained ' + sess.enemy.exp + ' experience! You have ' + sess.user.experience + ' experience!'},
                        {type: 'journey', author: 'Server', description: 'You gained ' + sess.enemy.gld + ' gold! You have ' + sess.user.gold + ' gold!'});
      //check for level up
      if(sess.user.experience >= sess.user.level * 25) {
        gameUtils.levelUp(sess);
        sess.history.push({type: 'journey', author: 'Server', description: 'You\'ve reached level ' + sess.user.level + '!'});
      }
      sess.user.hp = sess.user.maxhp;
      sess.user.mp = sess.user.maxmp;
      sess.enemy = null;
      sess.step = null;
      sess.history.push({type: 'lobby', author: 'Server', description: 'You happily return to the lobby...'});
      //save progress
      db.Warriors.update(
        {name: sess.name},
        {$set: {
    ...
    if(sess.user.hp == 0) {
      sess.history.push({type: 'journey', author: 'Server', description: 'You died...'});
      sess.enemy = null;
      sess.step = null;
      sess.user.hp = sess.user.maxhp;
      sess.user.mp = sess.user.maxmp;
      sess.history.push({type: 'lobby', author: 'Server', description: 'You are carried back to the lobby...'});
    }
  }
  if(!willRender)
    res.render('index', {history: sess.history, user: sess.user});
